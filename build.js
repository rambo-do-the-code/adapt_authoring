...
installHelpers.checkPrimaryDependencies(function(error) {
  ...
  installHelpers.getLatestFrameworkVersion(function(error, latestFrameworkTag) {
    ...
    console.log('');
    if(!fs.existsSync('conf/config.json')) {
      console.log('Found an existing config.json file. Please install it by run command node install.js');
      return;
    }
    USE_CONFIG = true;
    start();
...
function createMasterTenant(callback) {
  console.log(configResults.masterTenantID);
  /*
  ...
  */
  let folderPath = path.join(app.configuration.tempDir, configResults.masterTenantID.toString());
  fs.remove(folderPath, error =>{
    if(error){
    }
    fs.mkdirs(folderPath, function (err) {
      ...
      var tenantFrameworkDir = path.join(app.configuration.tempDir, configResults.masterTenantID.toString(), "adapt_framework");
      fs.exists(tenantFrameworkDir, function(exists) {
        ...
        installHelpers.installFramework({
          repository: app.configuration.getConfig('frameworkRepository'),
          revision: app.configuration.getConfig('frameworkRevision'),
...
function getSourceCodePlugin(callback){
  var tenantFrameworkDir = path.join(app.configuration.tempDir, configResults.masterTenantID.toString(), "adapt_framework");
  fs.readJSON(path.join(tenantFrameworkDir, "adapt-types.json"),function (error, json) {
    if (error) {
    ...
    async.eachSeries(
      plugins,
      function (pluginName, callbackPlugin) {
        let pluginType = json.dependencies[pluginName];
        if(pluginType ="*"){
          pluginType = "component";
        }
        console.log("Checking plugin "+pluginName + "with type "+pluginType);
        app.contentmanager.getContentPlugin(pluginType, (error, plugin) => {
          if (error) {
            return callbackPlugin(error);
          }
          app.db.retrieve(plugin.getPluginType(), {
            ...
            try{
              if(installedPlugin){
                console.log("Pulling source code "+pluginName + " with version "+ installedPlugin.version);
                var version = installedPlugin.version;
                app.bowermanager.installPlugin(pluginName, version, ()=>{
                  console.log("Finish pulling source code "+ pluginName);
                  callbackPlugin();
                });
              }else{
                console.log("Cancel pulling source code "+ pluginName);
                callbackPlugin();
              }
            }catch(e){
              console.log("install plugin "+pluginName + " was fail");
              console.log(e);
              callbackPlugin(e);
            }
      ...
      function (error) {
        ...
        console.log("====Finish pulling source code  all component====");
        callback();
      }
    );
