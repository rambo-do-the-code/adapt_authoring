  ...
  fs.readFile(filePath, function (err, data) {
    if (err) {
      ...
      try {
        ...
        config.conf.dbConnectionUri = process.env.DB_URI || config.conf.dbConnectionUri;
        config.conf.dbName = process.env.DB_NAME || config.conf.dbName;
        config.conf.masterTenantID = process.env.TENANT_ID || config.conf.masterTenantID ;
        logger.log('info',"Database Uri:"+config.conf.dbConnectionUri.substring(1,20));
        logger.log('info',"Database Name:"+config.conf.dbName);
      } catch (e){
...
Configuration.prototype.setConfig = function (name, value) {
  if (name && typeof name === 'string') {
    if (name == "dbConnectionUri") {
      this.conf.dbConnectionUri = process.env.DB_URI || value;
    } else if (name == "dbName") {
      this.conf.dbName = process.env.DB_NAME || value;
    } else if (name == "masterTenantID") {
      this.conf.masterTenantID = process.env.TENANT_ID || value;
    }
