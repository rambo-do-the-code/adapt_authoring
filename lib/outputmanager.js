...
OutputManager.prototype.setupRoutes = function () {
  ...
  rest.post('/integration/sync/course/s3',  function (req, res, next) {

    var mode = Constants.Modes.Publish;
    const apiKey = req.headers['x-api-key'];

    // now we commit simple api key to test
    if (apiKey !== 'onlytestapikey') {
      return res.status(403).json({error: 'Forbidden: Invalid x-api-key'});
    }

    // set default passport
    req.session.passport = req.body.passport

    logger.log('info', 'sync course to s3 userId ' + req.body.userId + ' courseId ' + req.body.courseId);

    that.publish('adapt', req.body.courseId, mode, req, res, function (error, result) {
      ...
      return res.json({success: true, payload: result});
    }, 'true', req.body.version,req.body.userId);
  });
