...
exports = module.exports = {
  ...
  init: function (app) {
    ...
    rest.post('/integration/user', function (req, res, next) {
      ...
      logger.log('info', 'Sync user account: ' + userData.email);

      if (apiKey !== X_API_KEY) {
        return res.status(403).json({error: 'Forbidden: Invalid x-api-key'});
      }

      // fetch the role ID for the role name (e.g., "Authenticated User")
      getRoleIdByName(userData.roleName, function(err, roleId) {

        if (err) {
          return res.status(500).json({ error: 'Failed to fetch role ID: ' + err });
        }

        userData.role =  roleId.toString();
    ...
    function getRoleIdByName(roleName, callback) {
