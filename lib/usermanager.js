...
var X_API_KEY = process.env.X_API_KEY || 'onlytestapikey';
/*
 * CONSTANTS
 */
...
exports = module.exports = {
  ...
  createNewUserSync: function(user, callback) {
    var auth = require('./auth');
    ...
    auth.getAuthPlugin(configuration.getConfig('auth'), function (err, plugin) {
      ...
      plugin.internalRegisterUser(false, user, function(error, result) {
        ...
        app.rolemanager.assignRoleByName(user.roleName, result._id, function (err, userRecord) {
          if (err) {
            return callback(err);
          }
  ...
  init: function (app) {
    ...
    rest.get('/integration/user/:email', function (req, res, next) {
      var email = req.params.email;

      logger.log('info', 'get user account: ' + email);

      self.retrieveUser({ email: email }, { populate: { 'roles' : 'name', '_tenantId' : 'name' } }, function (err, usr) {
        if (err) {
          return next(err);
        }
    ...
    rest.post('/integration/user', function (req, res, next) {
      const apiKey = req.headers['x-api-key'];
      const userData = req.body

      logger.log('info', 'Sync user account: ' + userData.email + ' role:' + userData.roleName);

      if (apiKey !== X_API_KEY) {
        return res.status(403).json({error: 'Forbidden: Invalid x-api-key'});
      }

      self.createNewUserSync(userData, function (err, result) {
        if (err) {
          res.statusCode = 500;
          return next(err);
        }
