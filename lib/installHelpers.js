...
var DEFAULT_GITHUB_ORG = "rambo-do-the-code"; // used to pull releases from
...
var DEFAULT_THEME_REPO = `https://github.com/${DEFAULT_GITHUB_ORG}/adapt-contrib-vanilla.git`;
...
function handleDependencyErrors(callback) {
  return function (error) {
    if (error && optimist.argv.skipDependencyCheck) {
      logger.log(
        "warn",
        `${error.message} The application may not function as intended.`
      );
      callback();
    } else {
      callback(error);
...
function checkPrimaryDependencies(callback) {
  checkDependencies(
    [checkNodeVersion, checkGitInstalled, checkGithubConnection],
    handleDependencyErrors(callback)
  );
}
function checkSecondaryDependencies(callback) {
  checkDependencies(
    [checkDbConnection, checkSmtpConnection],
    handleDependencyErrors(callback)
  );
}
  ...
  async.each(
    ...
    (err) => {
      callback(
        hasErrored
          ? new Error(
              "Some of the prerequisites could not be found, see above for details."
            )
          : null
      );
    }
...
function checkNodeVersion(callback) {
  ...
  if (!semver.satisfies(installedVersion, requiredVersion)) {
    return callback(
      null,
      chalk.yellow(
        `You are using Node.js ${installedVersion} which is not supported by Adapt. If you encounter issues, please change to version ${requiredVersion}.`
      )
    );
  }
  callback();
...
function checkGitInstalled(callback) {
  execCommand("git --version", {}, (error) => {
    callback(
      error ? "git could not be found, please check it is installed" : null
    );
  });
}
...
function checkGruntCliInstalled(callback) {
  execCommand("grunt --version", {}, (error) => {
    callback(
      error
        ? "grunt-cli could not be found, please check it is installed"
        : null
    );
  });
}
...
function checkDbConnection(callback) {
  database.checkConnection((error) => {
    callback(
      error
        ? `Couldn't connect to the database, please check the connection details are valid`
        : null
    );
  });
}
...
function checkSmtpConnection(callback) {
  ...
  new mailer().testConnection((error) =>
    callback(null, (error && error.message) || null)
  );
}
...
function getInstalledServerVersion(callback) {
  fs.readJSON("package.json", (error, pkg) => {
    if (error) {
      return callback(
        new Error(`Cannot determine authoring tool version\n${error}`)
      );
    }
...
function getInstalledFrameworkVersion(callback) {
  fs.readJSON(path.join(getFrameworkRoot(), "package.json"), (error, pkg) => {
    if (error) {
      return callback(
        new Error(`Cannot determine framework version\n${error}`)
      );
    }
    callback(null, pkg.version);
  });
...
function getInstalledVersions(callback) {
  async.parallel(
    [exports.getInstalledServerVersion, exports.getInstalledFrameworkVersion],
  ...
  async.parallel(
    [
      ...
      async.apply(
        ...
        installedVersions.adapt_framework
      ),
    ],
    function (error, results) {
      callback(error, [
...
function getUpdateData(callback) {
  async.waterfall(
    [exports.getInstalledVersions, exports.getLatestVersions],
    function (error, results) {
      if (error) {
      ...
      var updateData = {};
      if (
        results[1].adapt_authoring &&
        semver.lt(results[0].adapt_authoring, results[1].adapt_authoring)
      ) {
        updateData.adapt_authoring = results[1].adapt_authoring;
      }
      if (
        results[1].adapt_framework &&
        semver.lt(results[0].adapt_framework, results[1].adapt_framework)
      ) {
...
function checkLatestAdaptRepoVersion(repoName, versionLimit, callback) {
  ...
  var _getReleases = function (done) {
    ...
    if (GITHUB_USER && GITHUB_TOKEN) {
      const authHash = Buffer.from(`${GITHUB_USER}:${GITHUB_TOKEN}`).toString(
        "base64"
      );
  ...
  var _requestHandler = function (error, response, body) {
    if (response) {
      ...
      if (
        ...
        error = `You have exceeded GitHub's request limit of ${
          response.headers["x-ratelimit-limit"]
        } requests per hour. Please wait until at least ${reqsReset.toTimeString()} before trying again.`;
    ...
    if (error) {
      return callback(
        new Error(`Couldn't check latest version of ${repoName}. ${error}`)
      );
    }
    ...
    async.someSeries(
      ...
      function (release, cb) {
        ...
        var satisfiesVersion =
          !versionLimit || semver.satisfies(release.tag_name, versionLimit);
      ...
      function (error, satisfied) {
        if (!satisfied) {
          ...
          error = new Error(
            `Couldn't find any releases compatible with specified framework version (${versionLimit}), please check that it is a valid version.`
          );
        }
        if (error) {
          return callback(error);
...
function installFramework(opts, callback) {
  if (arguments.length !== 2 || !opts.directory) {
    return callback(
      new Error("Cannot install framework, invalid options passed.")
    );
  }
  if (!opts.repository) {
  ...
  async.applyEachSeries(
    [cloneRepo, updateFramework, cloneTheme],
    opts
  )(callback);
}
...
function checkOptions(opts, action, callback) {
  if (!opts) {
    return callback(
      new Error(`Cannot ${action} repository, invalid options passed.`)
    );
  }
  if (!opts.repository) {
    return callback(
      new Error(`Cannot ${action} repository, no repository specified.`)
    );
  }
  if (!opts.directory) {
    return callback(
      new Error(
        `Cannot ${action} ${opts.repository}, no target directory specified.`
      )
    );
  }
  callback();
...
function cloneRepo(opts, callback) {
  checkOptions(opts, "clone", function (error) {
    ...
    fs.remove(opts.directory, function (error) {
      ...
      execCommand(
        `git clone ${opts.repository} --origin ${REMOTE_NAME} "${opts.directory}"`,
        function (error) {
          hideSpinner();
          if (error) {
            return callback(error);
...
function cloneTheme(opts, callback) {
  const repository = DEFAULT_THEME_REPO;
  const themeDirectory = `"${opts.directory}/src/theme/adapt-contrib-vanilla"`;
  checkOptions(opts, "clone", function (error) {
    if (error) {
      return callback(error);
    }
    showSpinner(`Cloning ${repository}`);
    fs.remove(
      `${opts.directory}/src/theme/adapt-contrib-vanilla`,
      function (error) {
        if (error) {
          hideSpinner();
          return callback(error);
        ...
        execCommand(
          `git clone ${repository} --origin ${REMOTE_NAME} ${themeDirectory} && cd ${themeDirectory} && git checkout develop`,
          function (error) {
            hideSpinner();
            if (error) {
              console.log(error);
            ...
            log(`Cloned ${themeDirectory} successfully.`);
            callback();
          }
        );
      }
    ...
    execCommand(
      ...
      function (error) {
        ...
        if (
          error &&
          error.indexOf(
            `'${REMOTE_NAME}' does not appear to be a git repository`
          ) > -1
        ) {
          error = new Error(
            `Remote with name '${REMOTE_NAME}' not found. Check it exists and try again.`
          );
        }
        callback(error);
      }
...
function updateRepo(opts, callback) {
  fetchRepo(opts, function (error) {
    ...
    checkOptions(opts, "update", function (error) {
      ...
      var shortDir =
        opts.directory.replace(configuration.serverRoot, "") || opts.directory;
      showSpinner(`Updating ${shortDir} to ${opts.revision}`);

      execCommand(
        `git reset --hard && git checkout ${opts.revision}`,
        {
          cwd: opts.directory,
        },
        function (error) {
...
function updateFrameworkPlugins(opts, callback) {
  if (arguments.length !== 2) {
    return callback(
      new Error(
        "Cannot update Adapt framework plugins, invalid options passed."
      )
    );
  }
  if (!opts.directory) {
    return callback(
      new Error(
        "Cannot update Adapt framework plugins, no target directory specified."
      )
    );
  }
...
function purgeCourseFolder(opts, callback) {
  if (arguments.length !== 2) {
    return callback(
      new Error("Cannot remove course folder, invalid options passed.")
    );
  }
  if (!opts.directory) {
    return callback(
      new Error("Cannot remove course folder, no target directory specified.")
    );
  }
...
function installDependencies(opts, callback) {
  ...
  fs.remove(path.join(cwd, "node_modules"), function (error) {
    ...
    fs.remove(path.join(cwd, "src", "core"), function (error) {
      ...
      execCommand(
        "npm install --unsafe-perm=true --loglevel error --production",
        { cwd },
        function (error) {
          hideSpinner();
          if (error) {
