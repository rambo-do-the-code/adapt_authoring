...
MongooseDB.prototype.connect = function(db) {
  ...
  if ('object' === typeof db) {
    ...
    dbName = process.env.DB_NAME || configuration.getConfig('dbName');
  }

  // Retrieve the user credentials and options.
  dbUser = process.env.DB_USER ||  configuration.getConfig('dbUser');
  dbPass = process.env.DB_PASS ||  configuration.getConfig('dbPass');
  dbReplicaset = configuration.getConfig('dbReplicaset');
  dbConnectionUri = process.env.DB_URI ||  configuration.getConfig('dbConnectionUri');
